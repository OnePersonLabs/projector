...
The keys live in `.codex/settings.json` (project) or `/home/zethj/dev/projector/.codex/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/home/zethj/dev/projector/.codex/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
