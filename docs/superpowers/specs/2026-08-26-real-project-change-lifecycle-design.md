...
This initial lifecycle may reuse a current canonical ArchitectureDecision, or it may record a bounded deferral for a `material-soon` or `deferable` concern. It cannot accept a new architecture decision. A new accepted decision requires the existing canonical decision, authority, governance, and consequence-compilation transaction and is outside this slice. A `blocking-now` concern without a current canonical decision also blocks.

A bounded deferral includes rationale, an explicit reconsideration condition, a validity horizon, preserved options, and forbidden commitments. It is authenticated operational evidence, never affirmative architecture authority. Its consequences can only narrow the plan: they become assumptions, forbidden writes, unavailable actions, and a reconsideration query in the StateBinding. Architecture preflight discovers/deduplicates the concern from requested facets plus repository evidence, verifies the bounded deferral, compiles those narrowing consequences, and blocks if the disposition is missing, stale, or attempts to authorize a technology choice.
