...
export async function createTrial(directory: string, trajectory: Trajectory, mode: TrialMode): Promise<Trial> {
  ...
  const history = trajectory === 'preview'
    ? 'A preview-specific strategy is introduced for an intermediate audit, then withdrawn. The final intent is one playback and preview route, while general evidence retention survives.'
    : trajectory === 'shared'
  ...
  if (mode === 'evolved' && trajectory !== 'preview') {
