...
export function stageArtifacts(mode: TrialMode, trajectory: Trajectory) {
  const existingStrategy = mode === 'evolved' && trajectory !== 'preview';
  const liveDesign = design(existingStrategy ? 'evolved' : 'direct', trajectory);
  ...
  return {
    ...
    initial: {
      ...
      [`${prefix}/specs/playback/events/spec.md`]: requirementDelta(mode === 'evolved' ? 'The runtime SHALL implement a preview strategy and retained history owner with previewCount(), counting only preview calls while preserving event provenance and independently retained evidence.' : finalMeaning),
