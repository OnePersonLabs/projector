...
import { CONTENT_HASH_PREFIX, canonicalJson, hashFramedDomain, type ArchitectureConcern, type ArchitectureDecision, type ContentHash, type CoverageSnapshot, type DecisionValidityAssessment, type ObservabilityClass, type RiskClass, type StateDigest } from "@projector/core";
import { analyzeLocalRepository, type LocalRepositoryAnalysis } from "@projector/analyzers";
import { BUILT_IN_REPRESENTATION_PROFILE_KEYS, canonicalRepresentationSourceFromSemanticChange, compileSemanticChange, compileSemanticChangePlan, createStateBinding, RepresentationCompiler } from "@projector/engine";
...
export { PROJECTOR_VERSION } from "./version.js";

export const DOGFOOD_DOCUMENT_VERSION = 1 as const;
export const OPERATIONAL_STATE_CACHE_VERSION = 1 as const;
export const MANDATORY_SLICE_VALIDATOR_VERSION = "1" as const;
export const CLI_REPRESENTATION_MEASUREMENT_PROFILE_IDS = Object.freeze({
  tokenizer: "projector.whitespace@1",
  utility: "projector.instruction-cost@1",
} as const);
...
function defaultOperationalCliPort(): OperationalCliPort {
  return { authenticate: async (report) => validateOperationalReport(report), run: async ({ command, repositoryRoot, clean, policy, signal, allowPersistence, maximumEvents }) => {
    ...
    if (command === "recover") {
      ...
      if (clean && allowPersistence && policy.allowAutoMutation && !proof.recoveryFailure) { const state = await paths.resolveWrite(".projector/state.db"); await mkdir((await paths.resolveWrite(".projector")).realTarget, { recursive: true }); await writeFile(state.realTarget, `${canonicalJson({ version: OPERATIONAL_STATE_CACHE_VERSION, canonicalDigest, analysisDigest: secondHash })}\n`, "utf8"); }
    }
...
async function inspectDogfood(paths: RepositoryPathService): Promise<{ readonly canonicalDigest: ContentHash; readonly findings: Array<{ code: string; title: string; path?: string; severity: "error"; evidenceIds: string[] }> }> {
  ...
  if (document.version !== DOGFOOD_DOCUMENT_VERSION) findings.push({ code: "dogfood-version", title: `Unsupported canonical dogfood version ${String(document.version)}`, path: ".projector/dogfood.json", severity: "error", evidenceIds: [] });
...
function defaultChangePort(repositoryRoot: string): ChangeCliPort {
  ...
  const representationArtifacts = {
    put: async (contentHash: ContentHash, content: string) => {
      ...
      const path = join(representationRoot, `${contentHash.slice(CONTENT_HASH_PREFIX.length)}.txt`);
    ...
    get: async (contentHash: ContentHash) => {
      try { return await readFile(join(representationRoot, `${contentHash.slice(CONTENT_HASH_PREFIX.length)}.txt`), "utf8"); }
  ...
  const compileRepresentation = async (value: { readonly change: Awaited<ReturnType<typeof compileSemanticChange>>["change"]; readonly boundState: Awaited<ReturnType<typeof compileSemanticChange>>["boundState"] }) => {
    const tokenizer = { profileId: CLI_REPRESENTATION_MEASUREMENT_PROFILE_IDS.tokenizer, measure: (text: string) => text.trim() === "" ? 0 : text.trim().split(/\s+/u).length };
    const compiler = new RepresentationCompiler({ artifacts: representationArtifacts, tokenizer, utility: { profileId: CLI_REPRESENTATION_MEASUREMENT_PROFILE_IDS.utility, measure: ({ source, candidate, profileOverheadTokens }) => { const sourceCost = tokenizer.measure(source.statements.map(({ text }) => text).join("\n")); const candidateCost = tokenizer.measure(candidate) + profileOverheadTokens; return { netInstructionEfficiency: sourceCost - candidateCost, evidence: "deterministic total instruction payload cost including profile overhead" }; } } });
    const { projection } = await compiler.compileBest({ source: canonicalRepresentationSourceFromSemanticChange(value.change), binding: value.boundState, requestedProfileKey: BUILT_IN_REPRESENTATION_PROFILE_KEYS.agentCompact });
  ...
  const persist = async (prefix: "change" | "plan", body: ChangeRecord | PlanRecord): Promise<string> => {
    const hash = hashFramedDomain(`cli-immutable-${prefix}-selection`, body); const id = hash.slice(CONTENT_HASH_PREFIX.length);
  ...
  const load = async <T extends ChangeRecord | PlanRecord>(prefix: "change" | "plan", selector: string): Promise<T> => {
    ...
    if (hashFramedDomain(`cli-immutable-${prefix}-selection`, body).slice(CONTENT_HASH_PREFIX.length) !== id) throw new Error(`immutable ${prefix} selector content is corrupt`);
    return body;
  };
  ...
  return {
    change: async ({ intent, persist: shouldPersist }) => {
      ...
      const selector = shouldPersist ? await persist("change", record) : `change:semantic:${hashFramedDomain("cli-immutable-change-selection", record).slice(CONTENT_HASH_PREFIX.length)}`;
    ...
    plan: async (request) => {
      ...
      const planSelector = request.persist === false ? `plan:semantic:${hashFramedDomain("cli-immutable-plan-selection", record).slice(CONTENT_HASH_PREFIX.length)}` : await persist("plan", record);
