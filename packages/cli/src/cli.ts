...
export async function executeProjector(
  ...
  switch (parsed.command) {
    ...
    case "watch": case "ci": case "recover": case "verify": {
      const operations = options.operations ?? defaultOperationalCliPort(); const operationalReport = await operations.run({ command: parsed.command, repositoryRoot, clean: parsed.clean, policy, signal: options.signal ?? new AbortController().signal, ...(parsed.coverageRequest.budgetTokens === undefined ? {} : { maximumEvents: parsed.coverageRequest.budgetTokens }) }); if (!await operations.authenticate(operationalReport) || !validateOperationalReport(operationalReport)) return { exitCode: 6, output: "operational proof authentication failed", report: { policy, blocked: true } }; report = { policy, operationalReport }; exitCode = operationalReport.exitCode; break;
    }
    case "explain": {
...
function defaultOperationalCliPort(): OperationalCliPort {
  return { authenticate: async (report) => validateOperationalReport(report), run: async ({ command, repositoryRoot, clean, policy, signal, maximumEvents }) => {
    ...
    if (command === "recover") {
      ...
      if (clean && !proof.recoveryFailure) { const state = await paths.resolveWrite(".projector/state.db"); await mkdir((await paths.resolveWrite(".projector")).realTarget, { recursive: true }); await writeFile(state.realTarget, `${canonicalJson({ version: 1, canonicalDigest, analysisDigest: secondHash })}\n`, "utf8"); }
    }
