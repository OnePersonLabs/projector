...
import { CONTENT_HASH_PREFIX, canonicalJson, hashFramedDomain, type ContentHash, type ExecutionCapsule, type ExecutionPlan, type RepresentationProjectionRef, type StateBinding, type StateDigest } from "@projector/core";
...
export function hostSessionSelector(record: StoredHostSession): string { return `session:${record.contentHash.slice(CONTENT_HASH_PREFIX.length)}`; }
...
export async function loadBuiltHostSession(request: { readonly host?: "codex" | "claude"; readonly sessionSelector: string; readonly repositoryRoot: string }): Promise<StoredHostSession> {
  ...
  if (stored.kind !== "task17-host-session" || contentHash !== hashFramedDomain("task17-host-session", body) || contentHash.slice(CONTENT_HASH_PREFIX.length) !== match[1]) throw new Error("host session selector is unauthenticated");
