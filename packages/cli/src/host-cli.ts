...
export function createBuiltRunHostPort(): RunHostCliPort {
  const resolveSession = async (request: { readonly host: "codex" | "claude"; readonly sessionSelector: string; readonly repositoryRoot: string }): Promise<StoredHostSession> => {
    ...
    if (stored.host !== request.host || stored.repositoryRootHash !== hashFramedDomain("task17-host-repository-root", await realpath(request.repositoryRoot))) throw new Error("host session route/root mismatch");
  ...
  return { async resolve(request) { const stored = await resolveSession(request); return { authenticated: true, host: stored.host }; }, async run(request) {
    const stored = await resolveSession(request);
    if (await git(request.repositoryRoot, ["rev-parse", "HEAD"]) !== stored.plan.boundState.compiledAgainst.gitBase) throw new Error("host session is stale against the authoritative repository state");
    ...
    const result = await adapter.run({ sessionId: stored.sessionId, repositoryRoot: request.repositoryRoot, argv: request.argv, environment: request.environment, allowedEnvironmentKeys: Object.keys(request.environment), capsule: stored.capsule, binding: stored.plan.boundState, currentState: stored.plan.boundState.compiledAgainst, instructions: stored.instructions, signal: request.signal }, {
      bindingValidator: { validate: async (binding: StateBinding, currentState: StateDigest) => ({ status: binding.dependencyDigest === stored.plan.boundState.dependencyDigest ? "current" : "stale", currentState, changedValueDependencyIds: [], changedQueryDependencyIds: [], reasons: [] }) },
      ...
      reconcile: { run: async ({ launch }) => { if (beforeWithHead === undefined || afterWithHead === undefined) throw new Error("host observations are incomplete"); const committed = beforeWithHead.head === afterWithHead.head ? [] : (await git(request.repositoryRoot, ["diff", "--name-only", beforeWithHead.head, afterWithHead.head])).split(/\r?\n/u).filter(Boolean); const changedPaths = [...new Set([...committed, ...afterWithHead.paths])].sort(); const reconciliation = await reconcileToFixedPoint({ iterate: async () => ({ governedStateDigest: afterWithHead!.contentHash, materialChanged: false, fixedPointTerminal: true, details: { changedPaths } }) }, { maximumIterations: 2 }); const allowed = changedPaths.every((changed) => pathAllowed(stored.capsule, changed)); return { status: launch?.exitCode === 0 && allowed && reconciliation.converged ? "completed" as const : "failed" as const, changedPaths }; } },
    });
