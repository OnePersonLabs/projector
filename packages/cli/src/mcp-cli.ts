...
export function createBuiltMcpCliPort() {
  return { async start(request: { readonly repositoryRoot: string; readonly signal: AbortSignal; readonly sessionSelector?: string }): Promise<BuiltMcpLifecycle> {
    ...
    const representation = async (input: Readonly<Record<string, unknown>>, includeContent: boolean) => {
      if (session?.capsule.representation === undefined) return { status: "unavailable", reason: "no authenticated session representation is selected" };
      ...
      const contentHash = hashFramedDomain("representation-artifact", session.instructions.text);
      const valid = contentHash === session.capsule.representation.contentHash
        && JSON.stringify(session.instructions.representation) === JSON.stringify(session.capsule.representation);
      if (!valid) throw new Error("authenticated session representation artifact is invalid");
    ...
    const server = createBuiltProjectorMcpServer({ capability, read, representations: { preview: (input) => representation(input, true), validate: (input) => representation(input, false) }, controlled: (name) => ({ operation: name, risk: "R1", targets: (input) => ({ semanticScopes: typeof input.semanticScope === "string" ? [input.semanticScope] : [], writePaths: typeof input.path === "string" ? [input.path] : [] }), run: async (input) => { if (name !== "projector.apply_transform") throw new Error(`${name} is unavailable in the built local adapter`); if (typeof input.path !== "string" || typeof input.content !== "string") throw new Error("apply_transform requires path and content"); const paths = await RepositoryPathService.create(request.repositoryRoot); const target = await paths.resolveWrite(input.path); await writeFile(target.realTarget, input.content); return { status: "applied", path: target.canonicalPath }; } }) });
