...
export async function inspectRepositoryCoverage(repositoryRoot: string, request: RepositoryCoverageRequest, mode: "coverage" | "complete" | "cleanup" = "coverage") {
  ...
  const lanes = REQUIRED_COVERAGE_LANES.map((key): CoverageLaneEvidence => {
    ...
    if (key === "validation-evidence") return graph.lensCompilationUnknown === undefined ? known(key, ruleFindings.filter(({ status }) => status === "satisfied").length, ruleFindings.length, "Satisfied executable findings for the observed obligations only; this is not general behavioral or independent-test completeness.") : unavailable(key, graph.lensCompilationUnknown);
  ...
  const binding = createStateBinding({ compiledAgainst: currentState, valueDependencies: [...graph.valueDependencies(graph.entities.map(({ id }) => id)), { kind: "adapter", id: "projector.repository-coverage-observation", versionHash: hashFramedDomain("repository-coverage-observation", { state: currentState, scope: request.scope, lanes, questions }), role: "Canonical meaning, inventory membership and executable obligation observations" }], queryDependencies: decisionResult.dependencies });
