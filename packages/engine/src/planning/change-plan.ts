...
export interface ChangePacketProposal {
  ...
  readonly dependencies: readonly string[]; readonly validatorIds: readonly string[]; readonly convergence?: { readonly group: string; readonly maximumIterations: number };
}
...
export interface CompiledChangePacket { readonly key: string; readonly packet: Readonly<WorkPacket>; readonly capsule: Readonly<ExecutionCapsule>; readonly packetHash: ContentHash; readonly capsuleHash: ContentHash; readonly semanticOwnerIds: readonly string[]; readonly writeSelectors: readonly string[] }
...
function normalizeProposals(raw: readonly ChangePacketProposal[]): ChangePacketProposal[] {
  ...
  for (const item of raw) {
    ...
    const proposal = { ...structuredClone(item), unitIds: unique(item.unitIds), semanticOwnerIds: unique(item.semanticOwnerIds), writeSelectors: unique(item.writeSelectors), dependencies: unique(item.dependencies), validatorIds: unique(item.validatorIds) };
