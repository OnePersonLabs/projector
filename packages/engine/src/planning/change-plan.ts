import { canonicalJson, hashFramedDomain, type CompletionContract, type ContentHash, type ExecutionCapsule, type ExecutionPlan, type RepresentationProjectionRef, type SemanticChange, type StateBinding, type WorkPacket } from "@projector/core";
...
export async function compileSemanticChangePlan(
  ...
  ports: { readonly changes: { read(changeId: string): Promise<AuthenticatedChangePlanningInput> }; readonly packets: { compile(change: SemanticChange): Promise<AuthenticatedPacketProposalSet> }; readonly representations: { compile(input: AuthenticatedChangePlanningInput["value"]): Promise<RepresentationProjectionRef> } },
  ...
  if ([representation.projectionId, representation.profileId, representation.profileVersion, representation.contentHash, representation.preservationHash].some((value) => value.trim() === "")) throw new Error("semantic-change representation reference is incomplete");
