...
const representationText = "authenticated bounded instructions";
const representation = { projectionId: "representation:host", profileId: "profile:agent-compact", profileVersion: "1", contentHash: hashFramedDomain("representation-artifact", representationText), preservationHash: hashFramedDomain("preservation", "host") };
const representedCapsule = { ...capsule, representation } satisfies ExecutionCapsule;
...
describe("truthful Codex/Claude host adapters", () => {
  ...
  it("journals/snapshots before launch and reconciles final diff after crash or cancellation", async () => {
    ...
    const result = await adapter.run({ sessionId: "session", repositoryRoot: "/repo", argv: ["--fake"], environment: { SAFE: "1", SECRET: "drop" }, allowedEnvironmentKeys: ["SAFE"], capsule: representedCapsule, binding, currentState: state, instructions: { text: representationText, sourceHashes: [capsule.normativeKernelHash], representation }, signal: new AbortController().signal }, ports);
  ...
  it("accepts a dependency-local rebound only through the authenticated binding validator", async () => {
    ...
    await expect(adapter.run({ sessionId: "s", repositoryRoot: "/repo", argv: [], environment: {}, allowedEnvironmentKeys: [], capsule: representedCapsule, binding, currentState, instructions: { text: representationText, sourceHashes: [capsule.normativeKernelHash], representation }, signal: new AbortController().signal }, ports)).resolves.toMatchObject({ status: "completed" });
  });

  it("refuses free-form, missing, or mismatched representation instructions before host launch", async () => {
    const launch = vi.fn(async () => ({ exitCode: 0 }));
    const adapter = createCodexHostAdapter({ probe: { executable: async () => true, feature: async () => true } });
    const ports = { bindingValidator: { validate: async () => ({ status: "current" as const, currentState: state, changedValueDependencyIds: [], changedQueryDependencyIds: [], reasons: [] }) }, authority: { verify: async () => true }, journal: { prepare: async () => ({ id: "j", contentHash: hashFramedDomain("j", "1") }), finish: async () => undefined }, observe: { capture: async () => ({ state, paths: [], contentHash: hashFramedDomain("o", "1") }) }, launcher: { launch }, reconcile: { run: async () => ({ status: "completed" as const, changedPaths: [] }) } };
    await expect(adapter.run({ sessionId: "s", repositoryRoot: "/repo", argv: [], environment: {}, allowedEnvironmentKeys: [], capsule, binding, currentState: state, instructions: { text: representationText, sourceHashes: [capsule.normativeKernelHash], representation }, signal: new AbortController().signal }, ports)).rejects.toThrow(/capsule.*representation/iu);
    await expect(adapter.run({ sessionId: "s", repositoryRoot: "/repo", argv: [], environment: {}, allowedEnvironmentKeys: [], capsule: representedCapsule, binding, currentState: state, instructions: { text: "tampered", sourceHashes: [capsule.normativeKernelHash], representation }, signal: new AbortController().signal }, ports)).rejects.toThrow(/artifact.*hash|representation/iu);
