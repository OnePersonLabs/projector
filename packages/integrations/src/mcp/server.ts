...
export function createBuiltProjectorMcpServer(dependencies: { readonly capability: MutationCapabilityService; readonly read: Tool; readonly controlled: ControlledTool }) {
  return createProjectorMcpServer({
    ...
    controlled: Object.fromEntries(REQUIRED_PROJECTOR_CONTROLLED_TOOLS.map((name) => [name, { ...dependencies.controlled, run: (input: Readonly<Record<string, unknown>>) => dependencies.controlled.run({ ...input, toolName: name }) }])),
  });
}
