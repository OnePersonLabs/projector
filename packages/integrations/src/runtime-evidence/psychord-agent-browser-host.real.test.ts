...
import { createPsychordApplicationObserver, type PsychordDependencyPin, type PsychordObservationPlan } from "./psychord.js";
...
for (const caseName of ["keep-reload-replay", "save-failure"] as const) {
  real(`observes the real Psychord ${caseName} flow in owned Windows Chrome`, async () => {
    ...
    const result = await createPsychordApplicationObserver(host).observeApplication(boundPlan, { signal });
