...
import { executePacketPlan, type AuthenticatedPacketExecution, type ObservedImpact } from "./packet-coordinator.js";
...
const observation = (packetId: string, phase: "before" | "after" | "rollback", path = `src/${packetId}.ts`) => { const effective = phase === "rollback" ? "before" : phase; const value = { state: state(`${effective}:${packetId}`), pathContentHashes: { [path]: digest(`${effective}:${packetId}:content`) }, renames: [], deletedPaths: [], unitStates: { [`unit:${packetId}`]: "valid" as const }, canonicalEntityHashes: {}, externalStateHashes: {}, generatedArtifactHashes: {}, cleanWorkingTree: true, unknownCount: 0, divergenceCount: 0 }; return { value, contentHash: hashFramedDomain("authenticated-packet-observation", value) }; };
...
describe("packet execution coordinator", () => {
  it("authenticates packet-to-packet currentness and commits only after authoritative diff, validation, and durable artifacts", async () => {
    const events: string[] = []; let current = state("before:contract"); const input = envelope([packet("contract"), packet("consumer", ["contract"])]);
    const result = await executePacketPlan(input, {
      ...
      effect: { run: async ({ packet }) => { const value = { source: "transform", group: "transform" }; return { claimedChangedPaths: ["outside/forged.ts"], outputHash: digest(`output:${packet.id}`), author: { ...value, contentHash: hashFramedDomain("authenticated-effect-author", { ...value, packetId: packet.id }) } }; } },
      observe: { capture: async ({ packet, phase }) => { const observed = observation(packet.id, phase); const value = { ...observed.value, state: phase === "before" ? current : state(`after:${packet.id}`) }; current = value.state; return { value, contentHash: hashFramedDomain("authenticated-packet-observation", value) }; } },
      validate: { run: async ({ packet, postState }) => [validation(packet.id, postState)] },
      artifacts: { put: async (artifact) => { events.push("packetId" in artifact ? `artifact:${artifact.status}:${artifact.packetId}` : `artifact:${artifact.kind}`); return { contentHash: hashFramedDomain("packet-execution-artifact", artifact), replayed: false }; } },
    });
  ...
  it("detects same-path content mutation, rejects forged approval/independence, and replaces commit intent with failure truth", async () => {
    ...
    const rolledBack = await executePacketPlan(input, common);
    expect(rolledBack).toMatchObject({ status: "partial", recovery: "rolled-back", observedImpact: { changedPaths: [] }, attemptedImpact: { changedPaths: ["src/contract.ts"] } });
    expect(artifacts.map(({ status }) => status).slice(0, 2)).toEqual(["intent", "failure"]);
  ...
  it("rejects a declared SCC before effects when no staging adapter exists", async () => {
    ...
    let effects = 0;
    const ports = { lease: { acquire: async () => ({ assertOwned: async () => {}, release: async () => {} }) }, authority: { verify: async () => true }, validatorTrust, reconciliation, currentness: { validate: async () => ({ currentState: state("before"), valid: true, proofHash: digest("current") }) }, transaction: { begin: async () => ({ apply: async () => {}, commit: async () => {}, rollback: async () => {} }) }, effect: { run: async ({ packet }: { packet: WorkPacket }) => { effects += 1; const value = { source: "transform", group: "transform" }; return { claimedChangedPaths: [], outputHash: digest(`stable:${packet.id}`), author: { ...value, contentHash: hashFramedDomain("authenticated-effect-author", { ...value, packetId: packet.id }) } }; } }, observe: { capture: async ({ packet, phase }: { packet: WorkPacket; phase: "before" | "after" | "rollback" }) => observation(packet.id, phase) }, validate: { run: async ({ packet, postState }: { packet: WorkPacket; postState: StateDigest }) => [validation(packet.id, postState)] }, artifacts: { put: async (artifact: unknown) => ({ contentHash: hashFramedDomain("packet-execution-artifact", artifact), replayed: false }) } };
    await expect(executePacketPlan(input, ports)).rejects.toThrow(/SCC|staging/iu); expect(effects).toBe(0);
  });
});
