...
export const WATCH_CHECKPOINT_VERSION = 1 as const;
export interface WatchCheckpoint { readonly version: typeof WATCH_CHECKPOINT_VERSION; readonly sequence: number; readonly pendingEvents: readonly WatchEvent[]; readonly lastResult: WatchResult }
...
const checkpointBody = (checkpoint: WatchCheckpoint): WatchCheckpoint => ({ version: WATCH_CHECKPOINT_VERSION, sequence: checkpoint.sequence, pendingEvents: checkpoint.pendingEvents, lastResult: checkpoint.lastResult });
export function authenticateWatchCheckpoint(checkpoint: AuthenticatedWatchCheckpoint): boolean { return checkpoint.version === WATCH_CHECKPOINT_VERSION && Number.isSafeInteger(checkpoint.sequence) && checkpoint.sequence >= 0 && checkpoint.contentHash === hashFramedDomain("authenticated-watch-checkpoint", checkpointBody(checkpoint)); }
