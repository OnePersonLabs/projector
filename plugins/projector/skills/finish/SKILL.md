...
1. Resolve the explicit change selection. “Finish” authorizes necessary verification and archive for the selected change; it does not authorize unrelated scope, integration into the user's branch, deployment, or external publication.
...
6. Tell the user what works, what was checked, and where the candidate branch is. State plainly that archiving leaves verified work on that branch; merging into their working branch is a separate action.
