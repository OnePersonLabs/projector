...
1. Read the target repository's agent instructions. Before choosing edit paths, resolve `../../scripts/projector-change.mjs` relative to this skill and run:
   ...
   Inspect the candidate overview, typed relevance reasons, applicable lens obligations, and unknown frontiers. Read the source and tests needed to resolve them. Select an existing meaning before using `--entity <id-or-key>` for focused detail. Preserve the returned context `id`; a free-text candidate is not an accepted identity. An empty result means knowledge is missing, not that no constraints exist.
