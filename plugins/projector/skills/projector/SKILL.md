...
2. Before choosing edit paths, call `projector.context` with `{ "request": "<requested outcome>" }`. Begin with the candidate overview; candidates are hypotheses. Select an existing meaning from the user's intent and repository evidence, then call context again with its stable ID in `entities` for focused detail. Inspect relevance reasons, obligations, unknowns, and disclosure counts. `deferredItems` and `requiredDisclosureExpansionIds` contain identity samples with exact total/included/omitted counts. Inspect required direct or governing meaning and semantic `requiredExpansionIds` before relying on focused context; use full detail when required identities were omitted from the sample. An empty sample with a nonzero total is not an absent constraint. Each branch's `fullEvidence.arguments` supplies an unabridged current-state CLI request; see `projector-change` for the bundled runtime route. This is a fresh focused request, not a read of the saved proof.
3. MCP context is read-only and unsaved. For reuse across sessions, use the `projector-change` skill's `context` wrapper and retain its returned `id`. Call `projector.validate` with `{ "contextId": "<saved id>" }` to check it later. Its binding status and current governance status answer different questions. Check finding and omission counts; use full CLI reconciliation when omitted findings matter. Stale knowledge, violated predicates, and unavailable checks are different outcomes. Only listed operational tools have a connected production implementation.
