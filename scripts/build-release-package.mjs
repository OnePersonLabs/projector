...
async function releaseVersion() {
  ...
  if (typeof manifest.version !== "string" || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u.test(manifest.version)) throw new Error("root Projector release version is invalid");
...
export async function buildReleasePackage(stagingRoot, packDestination) {
  ...
  for (const name of internalPackages) { const source = join(repositoryRoot, `packages/${name}`); const target = join(stagingRoot, `node_modules/@projector/${name}`); await mkdir(target, { recursive: true }); await cp(join(source, "dist"), join(target, "dist"), { recursive: true }); const manifest = JSON.parse(await readFile(join(source, "package.json"), "utf8")); manifest.version = version; manifest.private = false; for (const group of ["dependencies", "optionalDependencies", "peerDependencies"]) if (manifest[group] !== undefined) for (const [dependency, dependencyVersion] of Object.entries(manifest[group])) if (typeof dependencyVersion === "string" && dependencyVersion.startsWith("workspace:")) manifest[group][dependency] = version; await writeFile(join(target, "package.json"), `${JSON.stringify(manifest, null, 2)}\n`); }
  ...
  const nodeTypes = dirname(fileURLToPath(import.meta.resolve("@types/node/package.json"))); await mkdir(join(stagingRoot, "node_modules/@types"), { recursive: true }); await cp(nodeTypes, join(stagingRoot, "node_modules/@types/node"), { recursive: true }); const undiciTypes = await realpath(join(nodeTypes, "../../undici-types")); await cp(undiciTypes, join(stagingRoot, "node_modules/undici-types"), { recursive: true });
  ...
  const packageJson = { name: "projector", version, description: "Local semantic governance and change execution kernel", type: "module", engines: { node: ">=24 <25" }, bin: { projector: "./bin/projector.js" }, exports: Object.fromEntries(Object.keys(exportTargets).map((subpath) => { const base = `./exports/${subpath === "." ? "index" : subpath.slice(2).replaceAll("/", "-")}`; return [subpath, { types: `${base}.d.ts`, default: `${base}.js` }]; })), files: ["bin", "dist", "exports"], dependencies: Object.fromEntries([...bundledNames.map((name) => [name, version]), ["@types/node", "^24.13.3"], ["undici-types", "^7.18.2"], ["zod", "^4.0.15"]]), bundledDependencies: [...bundledNames, "@types/node", "undici-types", "zod"], publishConfig: { access: "public" } }; await writeFile(join(stagingRoot, "package.json"), `${JSON.stringify(packageJson, null, 2)}\n`);
