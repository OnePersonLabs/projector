...
describe("scoped Projector release package", () => {
  it("packs exactly @onepersonlabs/projector 2.1.0 with the projector executable and coherent internal versions", async () => {
    const root = await mkdtemp(join(tmpdir(), "projector-release-package-test-")); roots.push(root);
    const tarball = await buildReleasePackage(join(root, "projector-release-staging"), join(root, "packs"));
