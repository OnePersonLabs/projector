...
const manual = new Set(["EntityId", "Confidence", "ContentHash", "SourceClass", "GitRealizationLocator"]);
...
function enhancedSchemaExpression(name, node) {
  ...
  if (name === "ApplicationEvidencePredicateBinding") {
  ...
  if (textFields.some((item) => item.trim() === "" || item !== item.trim())) context.addIssue({ code: "custom", message: "application evidence binding identifiers must be nonblank normalized text" });
  if (value.assertionIds.length === 0 || new Set(value.assertionIds).size !== value.assertionIds.length) context.addIssue({ code: "custom", path: ["assertionIds"], message: "application evidence binding requires unique assertion identities" });
})`;
  }
  if (name === "Requirement") {
    return `${base}.superRefine((value, context) => {
...
const lines = [
  ...
  'import { z } from "zod";',
  'import { ConfidenceSchema, ContentHashSchema, EntityIdSchema, GitRealizationLocatorSchema, SourceClassSchema } from "./contracts.js";',
