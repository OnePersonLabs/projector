...
describe("release benchmark executable authority", () => {
  it("accepts only a repository root and derives fixed-seed analyzer plus real representation evidence", async () => { const root = await mkdtemp(join(tmpdir(), "benchmark-authority-")); try { await writeFile(join(root, "package.json"), '{"name":"benchmark-fixture","version":"1.0.0"}\n'); const result = await run(root); expect(result.metrics).toHaveLength(17); expect(result.rawObservations.map(({ class: fixtureClass }) => fixtureClass)).toEqual(["held-out", "mutation", "structural-variant", "representation"]); expect(result.rawObservations.every(({ outputHash }) => outputHash.startsWith("sha256:v1:"))).toBe(true); expect(result.metrics.find(({ id }) => id === "compact-context-net-negative")).toMatchObject({ value: 0 }); expect(result.metrics.find(({ id }) => id === "protected-dimension")).toMatchObject({ value: 0 }); await expect(execute(process.execPath, ["scripts/release-benchmark-authority.mjs", root, "{\"samples\":{}}"], { cwd: process.cwd() })).rejects.toBeDefined(); } finally { await rm(root, { recursive: true, force: true }); } });
