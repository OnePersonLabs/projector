...
/** Directly exercises canonical authority, projection fidelity, and compiler observability. */
export async function verifyRepresentationProjection({ core, engine, inventory, sourceRevision, worktreeDigest }) {
  ...
  return {
    ...
    fidelity,
    telemetry,
    negatives: { missingAuthority: authorityFailure, invalidCandidate: fidelityFailure, disconnectedTelemetry: telemetryFailure },
  };
}
async function verifyInstalledRepresentationDelivery(installedProjector, inventory, sourceRevision, worktreeDigest) {
  const [core, engine] = await Promise.all(["core", "engine"].map((name) => import(pathToFileURL(join(installedProjector, `exports/${name}.js`)).href)));
  const projection = await verifyRepresentationProjection({ core, engine, inventory, sourceRevision, worktreeDigest });
  const installed = await observeInstalledRepresentationLinks({ packagedRoot: installedProjector });
...
export async function observeInstalledRepresentationLinks({ packagedRoot }) {
  ...
  try {
    ...
    return {
      publicComposition: { capture, plan, association: delivered.association, fidelity: delivered.semanticFidelity },
      contentIntegrity: { delivered, wrongCapsule, tampered },
      packagedExport: { exportBytesHash: sha(operationBytes), severedPackage, restored },
      profileRecovery: recovered,
    };
...
export async function observeRepresentationProfileRecovery({ root, core, engine, controlPlane, proposal, reconcile }) {
  ...
  return { result, preservedHistory: [...before].map(([path, bytes]) => ({ path: path.slice(root.length), bytesHash: sha(bytes) })), negatives: { sameVersion, badBinding, missingArtifact } };
}
